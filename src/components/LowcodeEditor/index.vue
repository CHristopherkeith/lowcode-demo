...
const selectedComponent = computed(() => {
  ...
  return pageConfig.components.find((item) => item.id === selectedComponentId.value) || null
})
...
const handleDeleteComponent = (componentId: string) => {
  const index = pageConfig.components.findIndex((item) => item.id === componentId)
  if (index !== -1) {
...
const handleUpdateComponent = (updatedComponent: Component) => {
  const index = pageConfig.components.findIndex((item) => item.id === updatedComponent.id)
  if (index !== -1) {
